...
function buildCartoonUrl(publicId, params = {}) {
  ensureCloudinaryConfigured();
  
  const transformations = [
    // 1. Studio Lighting: Makes the image look less like a snapshot
    { effect: 'improve:outdoor' },
    
    // 2. The 3D Surface: Oil paint at a high level removes skin texture 
    // and creates the "plastic" look of a character model.
    { effect: 'oil_paint:40' }, 
    
    // 3. Subsurface Scattering: High vibrance mimics the way light 
    // hits animated characters.
    { effect: 'vibrance:50' },
    
    // 4. Subtle Stylization: VERY low line strength. 
    // We want the shading to define the face, not black outlines.
    { effect: 'cartoonify:10:60' }, 
    
    // 5. Soft Focus: A tiny blur helps blend the "painterly" look 
    // into a 3D surface.
    { effect: 'blur:50' }, 
    
    // 6. Final Sharpen: Brings back detail in the eyes only.
    { effect: 'sharpen:20' },
